...
def home_redirect(request):
    ...
    if request.session.get('just_registered', False):
        messages.success(request, f'Welcome, {user.name}! Your account has been successfully created.')
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            phone = form.cleaned_data['phone']  # Already normalized by clean_phone
            password = form.cleaned_data['password']
            
            # Authenticate using normalized phone number
            ...
            if user is not None:
                ...
                # Redirect to next parameter if exists, otherwise to home
